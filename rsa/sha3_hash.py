...
#----------UTILITY FUNCTIONS----------
    ...
    def get_hash(self, data: bytearray) -> int:
        copy_data = data.copy()
        padded_data = self.pad(copy_data)
        state = self.absorb(padded_data)
        return self.int_squeeze(state)
    
    def get_str_hash(self, data: bytearray) -> str:
        copy_data = data.copy()
        padded_data = self.pad(copy_data)
        state = self.absorb(padded_data)
        return self.str_squeeze(state)
    
    def get_bytes_hash(self, data: bytes) -> bytes:
        int_hash = self.get_hash(bytearray(data))
        
        int_size = (self.o // 8)
        return int_hash.to_bytes(length=int_size, byteorder='little')
...
#----------SQUEEZING----------
    def str_squeeze(self, state: list[list[int]]) -> int:
        str_output = ''
        for i in range(5):
            for j in range(5):
    ...
    def int_squeeze(self, state: list[list[int]]) -> int:
        bytes_res = []
        for i in range(5):
            for j in range(5):
                bytes_res.extend(state[j][i].to_bytes(8, 'little'))

        bytes_res = bytes_res[:self.o // 8]
        return int.from_bytes(bytes_res)

# encoder = Sha3_512_encoder()
# file = open('small.txt', 'rb')
# bbfile = file.read()
# bfile = bytearray(bbfile)
# print(encoder.get_str_hash(bfile))
# print(hex(encoder.get_hash(bfile)))
# print(encoder.get_bytes_hash(bbfile))
# print(hex(int.from_bytes(encoder.get_bytes_hash(bbfile), 'little')))